...
function run(str, code2) {
  return `${code2.open}${str.replace(code2.regexp, code2.open)}${code2.close}`;
}
function bold(str) {
...
function replaceEscapeSequences(string) {
  ...
  return string.replace(
    ...
    /[\x00-\x1f\x7f-\x9f]/g,
    (c) => "\\x" + c.charCodeAt(0).toString(16).padStart(
      2,
      "0"
    )
  );
...
function quoteString(string) {
  var _a;
  const quote = (_a = QUOTES.find((c) => !string.includes(c))) != null ? _a : QUOTES[0];
...
function inspectValueWithQuotes(value, inspectOptions) {
  const abbreviateSize = typeof inspectOptions.strAbbreviateSize === "undefined" ? STR_ABBREVIATE_SIZE : inspectOptions.strAbbreviateSize;
  const green2 = maybeColor(green, inspectOptions);
  switch (typeof value) {
    case "string": {
      const trunc = value.length > abbreviateSize ? value.slice(0, abbreviateSize) + "..." : value;
...
${closingDelimIndentation}`;
    ...
    if (stripColor(iContent).length > LINE_BREAKING_LENGTH || !inspectOptions.compact) {
...
function getClassInstanceName(instance) {
  ...
  if (typeof constructor == "function") {
    return (_a = constructor.name) != null ? _a : "";
  }
  return "";
}
...
function inspectRawObject(value, inspectOptions) {
  ...
  if (inspectOptions.sorted) {
    ...
    symbolKeys.sort(
      (s1, s2) => {
        var _a, _b;
        return ((_a = s1.description) != null ? _a : "").localeCompare((_b = s2.description) != null ? _b : "");
      }
    );
  }
  ...
  for (const key of stringKeys) {
    if (inspectOptions.getters) {
      ...
      const inspectedValue = error == null ? inspectValueWithQuotes(propertyValue, inspectOptions) : red2(`[Thrown ${error.name}: ${error.message}]`);
  ...
  inspectOptions.indentLevel--;
  const totalLength = entries.length + inspectOptions.indentLevel + stripColor(entries.join("")).length;
  if (entries.length === 0) {
    baseString = "{}";
...
function inspectFunction(value, inspectOptions) {
  var _a, _b;
  const cyan2 = maybeColor(cyan, inspectOptions);
  if (Reflect.has(value, denoCustomInspect) && typeof value[denoCustomInspect] === "function") {
    return String(value[denoCustomInspect](inspect, inspectOptions));
  }
  let cstrName = (_b = (_a = Object.getPrototypeOf(value)) == null ? void 0 : _a.constructor) == null ? void 0 : _b.name;
  if (!cstrName) {
    cstrName = "Function";
  }
  let suffix = ``;
...
function parseCss(cssString) {
  ...
  for (const [key, value] of rawEntries) {
    if (key == "background-color") {
      ...
      for (const lineType of value == null ? void 0 : value.split(/\s+/g)) {
      ...
      css.textDecorationLine = [];
      for (const arg of value == null ? void 0 : value.split(/\s+/g)) {
...
function colorEquals(color1, color2) {
  return (color1 == null ? void 0 : color1[0]) == (color2 == null ? void 0 : color2[0]) && (color1 == null ? void 0 : color1[1]) == (color2 == null ? void 0 : color2[1]) && (color1 == null ? void 0 : color1[2]) == (color2 == null ? void 0 : color2[2]);
}
function cssToAnsi(css, prevCss = null) {
  prevCss = prevCss != null ? prevCss : getDefaultCss();
  let ansi = "";
...
function inspectError(value, cyan2) {
  ...
  causes.shift();
  let finalMessage = (_a = refMap.get(value)) != null ? _a : "";
  if (AggregateError.prototype.isPrototypeOf(value)) {
    const stackLines = (_b = value.stack) == null ? void 0 : _b.split("\n");
    while (true) {
      const line = stackLines == null ? void 0 : stackLines.shift();
      if (/\s+at/.test(line)) {
        stackLines == null ? void 0 : stackLines.unshift(line);
        break;
    ...
    const aggregateMessage = value.errors.map((error) => inspectArgs([error]).replace(/^(?!\s*$)/gm, " ".repeat(4))).join("\n");
    finalMessage += aggregateMessage;
    finalMessage += "\n";
    finalMessage += stackLines == null ? void 0 : stackLines.join("\n");
  } else {
    finalMessage += value.stack;
  }
  finalMessage += causes.map((cause) => {
    var _a2, _b2;
    return "\nCaused by " + ((_a2 = refMap.get(cause)) != null ? _a2 : "") + ((_b2 = cause == null ? void 0 : cause.stack) != null ? _b2 : cause);
  }).join("");
  return finalMessage;
}
...
function inspectMap(value, inspectOptions) {
  const options = {
    ...
    entryHandler: (entry, inspectOptions2) => {
      const [key, val] = entry;
      inspectOptions2.indentLevel++;
      const inspectedValue = `${inspectValueWithQuotes(key, inspectOptions2)} => ${inspectValueWithQuotes(val, inspectOptions2)}`;
      inspectOptions2.indentLevel--;
      return inspectedValue;
    },
    group: false,
...
function inspectObject(value, inspectOptions, proxyDetails) {
  ...
  if (Reflect.has(value, denoPrivateCustomInspect) && typeof value[denoPrivateCustomInspect] === "function") {
    return String(
...
function _inspectValue(value, inspectOptions) {
  ...
  const gray2 = maybeColor(gray, inspectOptions);
  const cyan2 = maybeColor(cyan, inspectOptions);
  const bold2 = maybeColor(bold, inspectOptions);
  const red2 = maybeColor(red, inspectOptions);
  switch (typeof value) {
    case "string":
